...
A **First-Admission Record** ist maßgeblich für das Anhängen von Metadaten
außerhalb von Signed Document Verifizierungsprofil. Es MUST entsprechen
`schemas/first-admission-record.schema.json` und enthalten genau diese neun
Pflichtfelder: `protocolVersion`, `admissionRecordId`, `organizationId`,
`documentKind`, `signingHash`, `keyId`, `principal`, `trustedAcceptedAt`, Und
`acceptedBy`. `trustedAcceptedAt` ist das Organization-zugewiesener
vertrauenswürdiger Akzeptanzzeitpunkt. Der Datensatz ist kein Signed Document,
MUST NOT enthalten a `signature`, authentifiziert sich nicht und MUST NOT allein
aufgrund seiner eigenen Bytes akzeptiert werden.
...
Innerhalb eines Organization Admission Log ist der logische Such- und
Anhängeschlüssel `(organizationId, signingHash)`. Das Protokoll MUST
authentifiziert den akzeptierenden Dienst, beschränkt Schreibvorgänge auf von
Organization autorisierte Dienste, bewahrt die Nur-Anhänge-Integrität,
unterstützt eine autorisierende Suche, die zwischen Gefundenem und
autorisierendem Nichtvorhandensein unterscheidet, und stellt einen atomaren
Vorgang zum Anhängen oder Zurückgeben eines vorhandenen Schlüssels für diesen
logischen Schlüssel bereit. Nicht verfügbare, unbestimmte, nicht
authentifizierte, Integritätsfehler, Konflikte oder fehlgeschlagene
Commit-Ergebnisse MUST fehlgeschlagen. Ein vom Aufrufer bereitgestellter
Vertrauens-, Authentifizierungs- oder Integritätsboolean MUST NOT ersetzt das
typisierte Ergebnis des Adapters.
...
Jeder Fehler nach dem sechsstufigen Abschluss bei der Suche, der
Kandidatenvorbereitung, der Validierung vertrauenswürdiger Zeitintervalle, dem
Anhängen oder Zurückgeben vorhandener Dateien, dem Parsen zurückgegebener
Datensätze, der Schemavalidierung, der Bindung, dem Vergleich authentifizierter
Dienste usw Event selbstverankernd MUST Karte auf dem Draht zu
`AUTH_INVALID_SIGNATURE`. Die geschützte Audit-Diagnose MUST Bühne behalten
`admission` und einen stabilen Grund, ohne diesen Grund dem nicht
vertrauenswürdigen Anrufer preiszugeben.
