...
Nach der Dekodierung eines 64-Byte Ed25519 Unterschrift als `Renc || Senc`, ein
Prüfer MUST kanonisch entschlüsseln `Renc` als Edwards25519-Punkt und
-Anforderung `[L]R` der Identität gleichkommen. Der Identitätspunkt ist zulässig
für `R`. Der Prüfer MUST interpretieren `Senc` Als vorzeichenlose
Little-Endian-Ganzzahl ist „0“ erforderlich <= S <
L`, and MUST NOT reduce an out-of-range value modulo `L`. Noncanonical, off-curve, non-identity small-order, or mixed-order `R`encodings and out-of-range`S`
...
„validFrom <= t`, `validUntil`is absent or`t <
validUntil`, and `revokedAt`is absent or`t <
revokedAt`. A key whose `revokedAt`is equal to or earlier than`t` MUST be
