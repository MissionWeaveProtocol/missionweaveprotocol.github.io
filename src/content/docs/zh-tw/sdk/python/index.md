...
schema 檢查全部 56 個內建測試向量。協定儲存庫仍是規範性來源；
...
該命令產生一份規範化 JSON 報告；如果缺少任何必要行為，則以非零狀態退出。確定性場景涵蓋兩個同時進行的 Mission、共享 Worker、子任务、對等澄清、排程、僅限 checkpoint 的搶佔、復原、Coordinator 驗證和精確的人類核准。
