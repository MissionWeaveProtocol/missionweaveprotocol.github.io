...
NOT 将作为新验证或首次准入决策的当前证据提供。 Organization
...
1. 严格解析恰好一个 UTF-8
   JSON 值，并拒绝无效 UTF-8、字节顺序标记、尾随数据和重复的已解码对象成员名称；
2. 根据规范 Schema 验证完整的 Signed
   Document，包括必需的签名信封和受支持的算法；
3. 应用此验证配置文件，包括受保护时间的 UTC-`Z` 形式验证、未经转换的精确
   `createdAt` 相等性、预期签名者规则选择、`signature.value`
   的规范无填充 base64url 解码，以及对 64 字节 Ed25519 签名的 `Renc` 和 `Senc`
   严格验证；
4. 获取完整的 Organization 范围 Registry 证据，验证建立不可变签名密钥绑定、Organization 范围无重用和无别名不变量以及完整保留有效性历史所需的每个绑定，然后选择预期签名者下的固定密钥并验证其受保护时间区间，包括规范无填充 base64url 解码和对其 32 字节 Ed25519 公钥的严格点验证；
5. 恰好省略顶级 `signature`
   成员，拒绝[第 2 节](../foundations/#mwp-fnd-006)定义的 RFC
   8785 和 I-JSON 数据模型之外的值，并从接收到的值生成 JCS 签名字节，不对时间戳或数字执行超出 RFC
   8785 所需 binary64 序列化的转换；以及
6. 验证这些字节上的 Ed25519 签名。
